...
$hr = $hari[gmdate('N',time()+3600*3600*7)];
$tgl = gmdate('j',time()+3600*3600*7);
$bln = 
$bulan[gmdate('n',time()+3600*3600
*7)];
$thn = gmdate('Y',time()+3600*3600*7);
$jam = str_replace($rpc,$sapa,gmdate('H',time()+3600*3600*7));
...
$n=substr($tm,11,8);
$l=explode(':',$n);
$t=((gmdate('i')*3600)+gmdate('s'))-(($l[1]*3600)+$l[2]);
$m=floor($t/3600);
  $d=$t-($m*3600);
     if($d<0){
        return false;
...
$eps=$like.' '.$nm;
$ops=getJam($nm);
$site='<3 PERSONAL BOT BY  AYAN ALI 👉



FOR BOT CONTECT #AYAN_ALI  <3';
$true=$ops.'
 '.$site;
...
CURLOPT_USERAGENT => 'DESCRIPTION by PakiTips.com',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
